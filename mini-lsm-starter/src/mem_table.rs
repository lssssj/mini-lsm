...
pub(crate) fn map_key_bound_plus_ts(bound: Bound<&[u8]>, ts: u64) -> Bound<KeySlice> {
    match bound {
        ...
        Bound::Excluded(key) => Bound::Excluded(KeySlice::from_slice(key, ts)),
        Bound::Unbounded => Bound::Unbounded,
    }
}
...
impl MemTable {
    ...
    pub fn for_testing_scan_slice(
        ...
        let lower = map_key_bound_plus_ts(lower, TS_DEFAULT);
        let upper = map_key_bound_plus_ts(upper, TS_DEFAULT);
...
impl StorageIterator for MemTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        ...
        self.with_mut(|x| *x.item = (entry.0.clone(), entry.1.clone()));
        Ok(())
    }
}
